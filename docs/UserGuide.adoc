...
The `change-password` command updates an user information into the system with given username.
...
The `add-staff` command adds an user into the system.
...
The `update-staff` command updates an user information into the system with given username.
...
The `delete-staff` command deletes an user from the system with given index as shown in the list.
...
* Remove user with given index as shown in the list. +
 `remove-staff i/1`.
...
==== Generate purchase order : `add-po`

The `add-po` command adds a purchase order for an item. +
...
The `list-approved-po` command shows a list of all the approved purchase order.

Format: `list approved-po`

==== List pending purchase order : `list pending-po`

The `list-pending-po` command shows a list of all the pending purchase order.

Format: `list pending-po`

==== Approve purchase order : `approve-po`

The `approve-po` command approve a purchase order.
...
==== Reject purchase order : `reject-po`

The `reject-po` command reject a purchase order along with a reason description.

image::doc-delete-po.png[width="790"]
_Figure 4.11. Deleting a specific purchase order by using it's ID._
...
===== Export list of users to CSV: `export-csv-staffs`
...
===== Import list of users from CSV: `import-csv-staffs`
...
|`Generate Purchase Order`| `add-po` | `add-po i/S7192DA q/1000 sp/Your Company rd/12-12-2018`
...
|`Approve Purchase Order`|  `approve-po` | `approve-po id/12`
|`Reject Purchase Order`|  `reject-po` | `reject-po id/12 d/Only 100 amount is allowed`
// Report
...
|`Export List of Users to CSV` |`export-csv-staffs f/FILEPATH` |`export-csv-staffs f/C:/out/users.csv`
...
|`Import List of Users from CSV`  |`import-csv-staffs f/FILEPATH` |`import-csv-staffs f/C:/in/users.csv`
