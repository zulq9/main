...
Examples: Login the user with username as admin and password as password. +
`login u/admin p/password`

image::doc-login.png[width="790"]
_Figure 4.1. The user is required to login before proceeding._
...
* Change user's password with multiple given new passwords as nussoccat. Only the final input will be accepted. +
...
* Delete a user with an invalid index. +
`delete-staff 999`.
...
_Figure 4.14. Import successful prompt will be shown upon completion of import._
