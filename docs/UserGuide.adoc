...
Figure 2. The command box in Inventory Manager.
...
Figure X. Error shown on screen when creating a sale order with invalid SKU.
...
Figure X. Error shown on screen when deleting a sale order that cannot be found.
...
Figure X. All sales in Inventory Manager listed on screen.
