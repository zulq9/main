...
* Edit staff with given id, 2 in the list. Change name to Yao Teng Xiong and role as admin. +
 `update-staff 2 n/Yao Teng Xiong r/admin`.
...
`edit-user 2 u/yaotx n/Yao Teng Xiong r/admin`.
...
of 10 or less will appear as can be seen in the screenshot below.

==== Editing an item : `edit-item`

The `edit-item` command edits an existing item in the Inventory Manager. +
It also edits all the purchase orders that contains the existing item to the edited item if the sku is changed. +
This command is available to *members* only. +
...
* Items matching at least one keyword will be returned (i.e. `OR` search). e.g. `Iphone 10` will return `IPhone X`, `Samsung 10`
****

The following demonstrates entering the `find-item` command in the command box with the keyword `LG`, followed by kbd:[Enter].
...
* Items matching at least one keyword will be returned (i.e. `OR` search). e.g. `iphone sn` will return
items with SKUs containing the SKUs `iphone-s9-9234`, `SN-1234`
****

The following demonstrates entering the `find-item-sku` command in the command box with the keyword `lg-g7`, followed by kbd:[Enter].
...
[TIP]
You can combine both conditions or use only one condition to filter your products.

The following demonstrates entering the `filter-item` command in the command box with the condition `p/<100`, followed by kbd:[Enter].
...
It also deletes all purchase orders that contains the deleted item. +
This command is available to *members* only. +
...
The `list-po` command shows a list of all the purchase order. +
This command is available to *members* only. +
...
The `add-po` command adds a purchase order for an item based on it's sku. +
The default status of the added purchase order is 'PENDING'
This command is available to *members* only. +
...
The `approve-po` command approves the specified purchase order from the purchase order list. +
Upon approving, the quantity stated in the purchase order will be added to the designated item. +
This command is available to *manager* and *admin* only. +
...
image::doc-delete-po.png[width="790"]
_Figure 4.11. Approves the specified purchase order by the it's index in the list._

==== Rejecting a purchase order : `reject-po`

The `reject-po` command rejects the specified purchase order from the purchase order list. +
This command is available to the *manager* and *admin* only.
...
The `edit-po` command edits an existing purchase order in the Inventory Manager. +
This command is available to *members* only. +
...
* `edit-po 2 q/100` +
Updates the quantity of the 3rd purchase order to `100`.
...
The `delete-po` command deletes the specified purchase order from the purchase order list. +
This command is available to *members* only. +
...
All the command in this section is available to *admins* only. +
...
The `export-csv-sales` command export the sales report in CSV format to the filepath given.

Format: `export-csv-sales f/FILEPATH`

===== Exporting list of items to CSV : `export-csv-items`

The `export-csv-items` command export the list of items in CSV format to the filepath given.

image::doc-export-csv-item.png[width="790"]
_Figure 4.12. Export successful prompt will be shown upon completion of export._

Format: `export-csv-items f/FILEPATH`

===== Exporting list of users to CSV: `export-csv-staffs`

The `export-csv-staffs` command export the list of users in CSV format to the filepath given.

Format: `export-csv-staffs f/FILEPATH`

===== Exporting list of purchase orders to CSV: `export-csv-orders`

The `export-csv-orders` command export the list of purchase orders in CSV format to the filepath given.

Format: `export-csv-orders f/FILEPATH`

==== Exporting entire data to .im file: `export-im`
(coming in v2.0) +
The `export-im` command export the entire data to the .im file specified by `FILEPATH` to quickly transfer the data between two *Inventory Manager* applications. +
...
The `import-csv-sales` command imports the sales report in CSV format from the filepath given.

Format: `import-csv-sales f/FILEPATH`

===== Importing list of items from CSV : `import-csv-items`

The `import-csv-items` command imports the list of items in CSV format from the filepath given.

Format: `import-csv-items f/FILEPATH`

===== Importing list of users from CSV: `import-csv-staffs`

The `import-csv-staffs` command imports the list of users in CSV format from the filepath given.

Format: `import-csv-staffs f/FILEPATH`

===== Importing list of purchase orders from CSV: `import-csv-orders`

The `import-csv-orders` command imports the list of purchase orders in CSV format from the filepath given.
...
|`Export Sales Report to CSV` |`export-csv-sales f/FILEPATH` |`export-csv-sales f/C:/out/sales.csv` | `Admin`
|`Export List of Items to CSV` |`export-csv-items f/FILEPATH` |`export-csv-items f/C:/out/items.csv` | `Admin`
|`Export List of Users to CSV` |`export-csv-staffs f/FILEPATH` |`export-csv-staffs f/C:/out/users.csv` | `Admin`
|`Export List of Sale Orders to CSV` |`export-csv-orders f/FILEPATH` |`export-csv-orders f/C:/out/orders.csv` | `Admin`
|`Export Entire Data to .im File` |`export-im f/FILEPATH` |`export-im f/C:/out/data.im` | `Admin`
|`Import Sales Report from CSV` |`import-csv-sales f/FILEPATH` |`import-csv-sales f/C:/in/sales.csv` | `Admin`
|`Import List of Items from CSV` |`import-csv-items f/FILEPATH` |`import-csv-items f/C:/in/items.csv` | `Admin`
|`Import List of Users from CSV`  |`import-csv-staffs f/FILEPATH` |`import-csv-staffs f/C:/in/users.csv` | `Admin`
|`Import List of Sale Orders from CSV`  |`import-csv-orders f/FILEPATH` |`import-csv-orders f/C:/in/orders.csv` | `Admin`
|`Import Entire Data From .im File` |`import-im f/FILEPATH` |`import-im f/C:/in/data.im` | `Admin`
|=====================================================================================================
...
Refers to everyone that is using the application; Non-member and member(Admin, Staff and Manager).

*Member*

Refers to Admin, Staff and Manager.
