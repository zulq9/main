...
Edits an existing item in the image book. +
...
Restores the image book to the state before the previous _undoable_ command was executed. +
Format: `undo`

[NOTE]
====
Undoable commands: those commands that modify the image book's content (`add`, `delete`, `edit` and `clear`).
====
...
Clears all entries from the image book. +
Format: `clear`
