...
* `VersionedInventory#commit()` -- Saves the current image book state in its history.
* `VersionedInventory#undo()` -- Restores the previous image book state from its history.
* `VersionedInventory#redo()` -- Restores a previously undone image book state from its history.
