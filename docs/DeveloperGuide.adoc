...
. Current user who is executing the command must be providing a valid username and password.
...
There are various places where `authenticateUser` can be executed. The pros and cons of each option is shown below.
...
There are various places where `edit-staff` can be executed. The pros and cons of each option is shown below.
